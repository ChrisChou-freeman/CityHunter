...
import (
	...
	"github.com/ChrisChou-freeman/CityHunter/gamecore/motivation"
...
func (gs *GameStart) initialTilesByLevelData() {
	for _, tileInfo := range gs.levelData.TileData {
		...
		case tileInfo["tile"] == tool.PLAYERTILE:
			gs.player = motivation.NewPlayer(&image.Point{X: tileInfo["X"], Y: tileInfo["Y"] - 10}, gs.levelData)
...
func (gs *GameStart) keyEvent() {
	if gs.keymap.IsKeyBackPressed() {
		tool.GAME_FUCTION = tool.GAME_MAIN
	}
}
